...
export function detectOSType(ua: string): string {
    let operatingSystemRulesPart: [string, RegExp][]= [
        ['windows', /Windows/],
        ['mac_os', /(Mac_PowerPC)|(Macintosh)/], 
        ['linux', /(Linux)|(X11)/]
    ]

    for (let ii = 0, count = operatingSystemRulesPart.length; ii < count; ii++) {
        const [os, regex] = operatingSystemRulesPart[ii];
        const match = regex.exec(ua);
        if (match) {
            return os;
        }
