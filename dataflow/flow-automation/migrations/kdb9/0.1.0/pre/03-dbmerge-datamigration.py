    ...
    try:
        ...
        if pk_column:
            ...
            if last_pk is not None:
                sql = f"SELECT {cols_str} FROM {tbl} WHERE {pk} > %s ORDER BY {pk} LIMIT %s"
                cursor.execute(sql, (last_pk, batch_size))
            else:
                sql = f"SELECT {cols_str} FROM {tbl} ORDER BY {pk} LIMIT %s"
...
def run():
    logger.info("="*50)
    logger.info("人大金仓(KingbaseES) 迁移工具")
    logger.info("="*50)
    logger.info(f"连接: {DB_CONFIG['host']}:{DB_CONFIG['port']}/{DB_CONFIG['database']}")
