...
do
    ...
    "d")
        ...
        helm template ./helm/agent-operator-app -n agent-operator-app -f./helm/agent-operator-app/values.yaml ;;
    "k")
        echo "====使用ktctl连接远程环境====";
        .ktctl/dev.sh;;
    "p")
    ...
    "b")
        ...
        cp -f ./server/infra/config/agent-operator-app.yaml "$config_dir/" || exit 1

        # 强制复制密钥文件
        echo "覆盖密钥文件到 $secret_dir"
        cp -f ./server/infra/config/agent-operator-app-secret.yaml "$secret_dir/" || exit 1
        echo "开始构建运行........."
