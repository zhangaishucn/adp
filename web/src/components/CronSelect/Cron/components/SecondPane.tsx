...
function SecondPane(props: any): JSX.Element {
  ...
  const aStartTobA = (
    <InputNumber disabled={currentRadio !== 3} min={0} max={59} value={offsetFrom} size="small" onChange={onChangeOffsetFrom} style={{ width: 100 }} />
  );
  const aStartTobB = (
    <InputNumber disabled={currentRadio !== 3} min={0} max={59} value={offset} size="small" onChange={onChangeOffset} style={{ width: 100 }} />
  );

  return (
    <RadioGroup name="radiogroup" value={currentRadio} onChange={onChangeRadio}>
      <Radio style={radioStyle} value={1}>
        {intl.get('CronSelect.everySecond')}
      </Radio>
      ...
      <Radio style={radioStyle} value={2}>
        <>
          {intl.get('CronSelect.from')}&nbsp;{aTobA}&nbsp;-&nbsp;{aTobB}&nbsp;{intl.get('CronSelect.excuteOnceEverySecond')}
        </>
      </Radio>
      ...
      <Radio style={radioStyle} value={3}>
        <>
          ...
          {aStartTobA}
          &nbsp;{intl.get('CronSelect.secondStart')}&nbsp;
          {aStartTobB}
          &nbsp;{intl.get('CronSelect.secondExcuteOnce')}
        </>
      </Radio>
