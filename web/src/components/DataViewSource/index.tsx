...
export const DataViewSource: React.FC<{
  ...
  onCancel: () => void;
  selectedRowKeys?: any[];
}> = ({ open, maxCheckedCount = 9999, onCancel, onOk, selectedRowKeys = [] }) => {
  ...
  const onCheckListChange = (checkedItem: any): void => {
    ...
    if (isExist) {
      ...
      if (checkedList.length >= maxCheckedCount) {
        setCheckedList([...checkedList.slice(checkedList.length - maxCheckedCount + 1), checkedItem]);
        return;
      }
    ...
    >
      <Splitter className={styles['splitter-box']}>
        <Splitter.Panel defaultSize={210} min={150} max={280} collapsible className={styles['panel-box']}>
          <DataViewGroup onSelect={onSelect} selectedKeys={selectedKeys} />
        </Splitter.Panel>
