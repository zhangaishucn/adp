...
export const getTableData = (schema: any, parentKey = '', location: string = ''): any[] => {
  const data: any[] = [];
...
/**
 * 根据 MCP名称从 tools 数组中提取入参信息
 * @param toolsArray 工具数组
 * @param toolName 工具名称
 * @returns 入参信息数组
 */
export function extractMCPSchemaParamsByToolList(toolsArray: any[], toolName: string): FlatParamInfo[] {
  ...
  for (const location in tool?.inputSchema?.properties) {
    const itemValue = tool?.inputSchema?.properties[location];
    const params = dereference(itemValue, tool?.inputSchema);
    getTableData(params, location, location);
    list.push({ name: location, type: itemValue.type, source: 'body', key: location, children: getTableData(params, location, location) });
  }
