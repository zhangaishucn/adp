...
// 编号规则：OR1xx为OpenSearch Resource特定创建测试
func TestOpenSearchResourceSpecificCreate(t *testing.T) {
	var (
		ctx       context.Context
	...
	Convey("OpenSearch Resource特定功能AT测试 - 创建 - 初始化", t, func() {
		...
		// ========== OpenSearch Resource正向测试（OR101-OR120） ==========
...
// TestOpenSearchResourceSpecificUpdate OpenSearch Resource特定功能AT测试 - 更新
// 编号规则：OR3xx为OpenSearch Resource特定更新测试
func TestOpenSearchResourceSpecificUpdate(t *testing.T) {
	var (
		ctx       context.Context
		config    *setup.TestConfig
		client    *testutil.HTTPClient
	...
	Convey("OpenSearch Resource特定功能AT测试 - 更新 - 初始化", t, func() {
		ctx = context.Background()
		...
		// ========== OpenSearch Resource更新测试（OR301-OR320） ==========
		...
		Convey("OR301: 同时更新多个字段", func() {
			...
			updatePayload := resourcefixtures.BuildUpdatePayload(originalData, map[string]any{
				"name":        fixtures.GenerateUniqueName("multi-update-os-resource"),
				"description": "同时更新多个字段的OpenSearch Resource测试",
...
// TestOpenSearchResourceSpecificDelete OpenSearch Resource特定功能AT测试 - 删除
// 编号规则：OR4xx为OpenSearch Resource特定删除测试
func TestOpenSearchResourceSpecificDelete(t *testing.T) {
	var (
		ctx       context.Context
		config    *setup.TestConfig
		client    *testutil.HTTPClient
	...
	Convey("OpenSearch Resource特定功能AT测试 - 删除 - 初始化", t, func() {
		ctx = context.Background()
		...
		// ========== OpenSearch Resource删除测试（OR401-OR420） ==========
		...
		Convey("OR402: 删除后可以创建同名resource", func() {
			// 创建resource
			payload := resourcefixtures.BuildCreatePayload(catalogID)
		...
		Convey("OR405: 批量删除多个resource", func() {
			// 创建3个resource
