...
// parse.go 是语法解析器。 generated_parser.y 和 generated_parser.y.go 是语法解析器定义
...
func ParseExpr(ctx context.Context, input string) (expr Expr, err error) {
	p := newParser(input)
...
// Lex应该返回token类型，并将token值放到放在lval中（与yacc的yySymType对应）。
...
func (p *parser) expectType(ctx context.Context, node Node, want ValueType, context string) {
	t := p.checkAST(ctx, node)
	if t != want {
...
func (p *parser) checkAST(ctx context.Context, node Node) (typ ValueType) {
	...
	case *AggregateExpr:
		...
		p.expectType(ctx, n.Expr, ValueTypeVector, "aggregation expression")
		if n.Op == TOPK || n.Op == BOTTOMK || n.Op == QUANTILE {
			p.expectType(ctx, n.Param, ValueTypeScalar, "aggregation parameter")
		}
		if n.Op == COUNT_VALUES {
			p.expectType(ctx, n.Param, ValueTypeString, "aggregation parameter")
		}

	case *BinaryExpr:
		lt := p.checkAST(ctx, n.LHS)
		rt := p.checkAST(ctx, n.RHS)
		...
		for _, l1 := range n.VectorMatching.MatchingLabels {
			for _, l2 := range n.VectorMatching.Include {
				...
				if l1 == l2 && n.VectorMatching.LeftJoin {
					p.addParseErrf(opRange(), "label %q must not occur in LeftJoin and GroupLeft clause at once", l1)
				}
			}
			// out_join时，需校验 out_join 和 select_left、select_right中的不重复
			for _, l2 := range n.VectorMatching.IncludeLeft {
				if l1 == l2 && n.VectorMatching.OutJoin {
					p.addParseErrf(opRange(), "label %q must not occur in OutJoin and SelectLeft clause at once", l1)
				}

				for _, l3 := range n.VectorMatching.IncludeRight {
					if l2 == l3 && n.VectorMatching.OutJoin {
						p.addParseErrf(opRange(), "label %q must not occur in SelectLeft and SelectRight clause at once", l1)
					}
				}
			}
			for _, l2 := range n.VectorMatching.IncludeRight {
				if l1 == l2 && n.VectorMatching.OutJoin {
					p.addParseErrf(opRange(), "label %q must not occur in OutJoin and SelectRight clause at once", l1)
				}
			}
		}
	...
	case *Call:
		...
		for i, arg := range n.Args {
			...
			p.expectType(ctx, arg, n.Func.ArgTypes[i], fmt.Sprintf("call to function %q", n.Func.Name))
		}

		// 调用检查方法
		if n.Func.SemanticCheck != nil {
			err := n.Func.SemanticCheck(ctx, n.Args)
			if err != nil {
				p.addParseErrf(n.PositionRange(), "semanticCheck failed beacuse %s", err.Error())
			}
		}

	case *ParenExpr:
		p.checkAST(ctx, n.Expr)

	case *UnaryExpr:
		if n.Op != ADD && n.Op != SUB {
			p.addParseErrf(n.PositionRange(), "only + and - operators allowed for unary expressions")
		}
		if t := p.checkAST(ctx, n.Expr); t != ValueTypeScalar && t != ValueTypeVector {
	...
	case *MatrixSelector:
		p.checkAST(ctx, n.VectorSelector)
...
func parseDuration(ds string) (time.Duration, error) {
	dur, err := convert.ParseDuration(ds)
	if err != nil {
		return 0, err
	}
	if dur == 0 {
...
func (p *parser) setTimestamp(e Node, ts float64) {
	...
	if timestampp != nil {
		...
		**timestampp = convert.FromFloatSeconds(ts)
	}
