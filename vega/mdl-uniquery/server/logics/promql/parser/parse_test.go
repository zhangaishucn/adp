...
var testExpr = []struct {
...
}{
	{
		...
		expected: &UnaryExpr{
			...
			Expr: &VectorSelector{
				...
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "some_metric"),
				},
		...
		input:  "2.5.",
		fail:   true,
		errMsg: `1:4: parse error: unexpected character: '.'`,
	}, {
		input:  "100..4",
		fail:   true,
	...
	},
	{
		...
		expected: &BinaryExpr{
			...
			LHS: &VectorSelector{
				Name: "foo",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "foo"),
				},
				PosRange: PositionRange{
					Start: 0,
					End:   3,
			...
			RHS: &VectorSelector{
				Name: "bar",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "bar"),
				},
				PosRange: PositionRange{
					Start: 6,
					End:   9,
		...
		expected: &BinaryExpr{
			...
			RHS: &VectorSelector{
				Name: "sum",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "sum"),
				},
				PosRange: PositionRange{
					Start: 6,
					End:   9,
		...
		expected: &BinaryExpr{
			...
			RHS: &BinaryExpr{
				...
				LHS: &VectorSelector{
					Name: "bla",
					LabelMatchers: []*labels.Matcher{
						MustLabelMatcher(labels.MatchEqual, labels.MetricName, "bla"),
					},
					PosRange: PositionRange{
						Start: 13,
						End:   16,
				...
				RHS: &VectorSelector{
					Name: "blub",
					LabelMatchers: []*labels.Matcher{
						MustLabelMatcher(labels.MatchEqual, labels.MetricName, "blub"),
					},
					PosRange: PositionRange{
						Start: 21,
						End:   25,
		...
		expected: &BinaryExpr{
			...
			LHS: &BinaryExpr{
				...
				RHS: &VectorSelector{
					Name: "baz",
					LabelMatchers: []*labels.Matcher{
						MustLabelMatcher(labels.MatchEqual, labels.MetricName, "baz"),
					},
					PosRange: PositionRange{
						Start: 19,
						End:   22,
			...
			RHS: &VectorSelector{
				Name: "qux",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "qux"),
				},
				PosRange: PositionRange{
					Start: 26,
					End:   29,
		...
		input: "foo * on(labels.test,labels.blub) bar",
		expected: &BinaryExpr{
			Op: MUL,
			LHS: &VectorSelector{
				Name: "foo",
			...
			VectorMatching: &VectorMatching{
				Card:           CardOneToOne,
				MatchingLabels: []string{"labels.test", "labels.blub"},
				On:             true,
			},
		},
	}, {
	...
	},
	{
		...
		expected: &VectorSelector{
			Name: "min",
			LabelMatchers: []*labels.Matcher{
				MustLabelMatcher(labels.MatchEqual, labels.MetricName, "min"),
			},
			PosRange: PositionRange{
				Start: 0,
				End:   3,
		...
		expected: &VectorSelector{
			...
			LabelMatchers: []*labels.Matcher{
				...
				MustLabelMatcher(labels.MatchEqual, labels.MetricName, "foo:bar"),
			},
			PosRange: PositionRange{
				Start: 0,
				End:   15,
		...
		// 修改原逻辑，支持a:b 和 a.b 的labels名称
		input: `some_metric{a:b="b"}`,
		expected: &VectorSelector{
			Name: "some_metric",
			LabelMatchers: []*labels.Matcher{
				MustLabelMatcher(labels.MatchEqual, "a:b", "b"),
				MustLabelMatcher(labels.MatchEqual, labels.MetricName, "some_metric"),
			},
			PosRange: PositionRange{
				Start: 0,
	...
	},
	{
		...
		expected: &MatrixSelector{
			VectorSelector: &VectorSelector{
				Name: "test",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "test"),
				},
				PosRange: PositionRange{
					Start: 0,
					End:   4,
	...
	},
	{
		...
		expected: &BinaryExpr{
			...
			LHS: &VectorSelector{
				Name: "a",
				LabelMatchers: []*labels.Matcher{
					MustLabelMatcher(labels.MatchEqual, labels.MetricName, "a"),
				},
				PosRange: PositionRange{
					Start: 0,
					End:   1,
	...
	},
	{
		...
		expected: &VectorSelector{
			Name: "start",
			LabelMatchers: []*labels.Matcher{
				MustLabelMatcher(labels.MatchEqual, labels.MetricName, "start"),
			},
			PosRange: PositionRange{
				Start: 0,
				End:   5,
		...
		expected: &VectorSelector{
			Name: "end",
			LabelMatchers: []*labels.Matcher{
				MustLabelMatcher(labels.MatchEqual, labels.MetricName, "end"),
			},
			PosRange: PositionRange{
				Start: 0,
				End:   3,
...
func TestParseExpressions(t *testing.T) {
	...
	for _, test := range testExpr {
		...
		if len(test.input) > 20 {
			length = 20
		}
		t.Run(test.input[0:length], func(t *testing.T) {
			expr, err := ParseExpr(testCtx, test.input)
...
func TestNaNExpression(t *testing.T) {
	...
	expr, err := ParseExpr(testCtx, "NaN")
	require.NoError(t, err)
...
func TestExtractSelectors(t *testing.T) {
	...
	for _, tc := range [...]struct {
		...
		expr, err := ParseExpr(testCtx, tc.input)
		require.NoError(t, err)
