...
type VectorSelector struct {
	...
	// 先注释掉，uniquery 查询不涉及存储，未来如果有需要再打开
...
func (vmc VectorMatchCardinality) String() string {
	...
	case CardManyToMany:
		return "many-to-many"
	case LeftJoinManyToMany:
		return "left_join many-to-many"
	case OutJoinManyToMany:
		return "out_join many-to-many"
	}
...
type VectorMatching struct {
	...
	// 新增的LeftJoin和On同级
	LeftJoin bool
	// left_join和group_left一起使用时，group_left关键字可被省略
	ExistGroupLeft bool

	// 新增的OutJoin和On同级
	OutJoin bool
	// out_join和group_left、group_right一起使用时，group_left、group_right 可选
	ExistGroupRight bool
	IncludeLeft     []string
	IncludeRight    []string
}
