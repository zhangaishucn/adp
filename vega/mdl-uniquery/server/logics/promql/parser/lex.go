...
// 关键函数NextItem。itemp为出参，即当前解析的token。
...
// 关键函数lexStatements，词法解析的核心逻辑。
// 匹配结果后，通过emit存入l.itemp。并返回下一个词法解析的state方法。
...
// labels 的标识符也支持标识符中包含 ':' 和 '.'
func lexIdentifier(l *Lexer) stateFn {
Loop:
	for {
		switch r := l.next(); {
		case isAlphaNumeric(r) || r == '.' || r == ':': // labels 的标识符也支持 : 和 .
			// absorb
		default:
			l.backup()
			l.emit(IDENTIFIER)
...
// is scanned.
// 为来支持存储在 opensearch 中的指标名为 a.b.c 的情况，扩展了 promql 语句中的指标标识符的定义情况
func lexKeywordOrIdentifier(l *Lexer) stateFn {
Loop:
	for {
		switch r := l.next(); {
		case isAlphaNumeric(r) || r == ':' || r == '.': // 指标标识符可支持点号 . 的情况
			// absorb.
		default:
			l.backup()
			word := l.input[l.start:l.pos]
			if kw, ok := key[strings.ToLower(word)]; ok {
				l.emit(kw)
			} else if !strings.Contains(word, ":") && !strings.Contains(word, ".") { // 指标标识符可支持点号 . 的情况
				l.emit(IDENTIFIER)
			} else {
