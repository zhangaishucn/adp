...
import { MinusOutlined, PlusOutlined, CenterOutlined, CopyOutlined } from "@applet/icons";
...
export const Editor = forwardRef<Instance, EditorProps>((props, ref) => {
    ...
    const [stepNodes, stepOutputs] = useMemo<
        [StepNodeList, StepOutputs]
    >(() => {
        ...
        function traverse(step: IStep, path: number[] = []) {
            switch (step.operator) {
                ...
                default: {
                    const [action, executor, extension] =
                        executors[step.operator] || [];
        ...
        if (value.length) {
            ...
            if (action?.allowDataSource && step.dataSource?.id) {
                const [action, extension] =
                    dataSources[step.dataSource.operator] || [];
                if (action) {
                    let nodeOutputs: Output[] = [];
                    ...
                    if (nodeOutputs.length) {
                        nodeOutputs.forEach((output) => {
                            outputs[`__${step.dataSource!.id}${output.key}`] =
                                output;
                        });
                    }
                }
    ...
    const validate = useEvent(async () => {
        ...
        if (platform === "console" && value.length === 1) {
            message.info(
                t("err.invalidParameter.node", "至少需包含一个执行节点")
            );
            return false;
        }
        let isValid = true;
                ...
                switch (node?.type) {
                    ...
                    case "comparator":
                        if (typeof node.comparator?.validate === "function") {
                            return await node.comparator.validate(
                                step.parameters
                            );
                        }
                        break;
    ...
    const contextValue = useMemo<EditorContextType>(() => {
        return {
            ...
            onConfigStep: (value, onFinish) =>
                setCurrentStep({ value, onFinish }),
    ...
    return (
        <div className={clsx(styles.editor, className)}>
                        ...
                        <TriggerConfig
                            ...
                            onFinish={(value) => {
                                ...
                                currentTrigger?.onFinish({
                                    ...value,
                                    ...((value?.operator === currentTrigger?.value?.operator) && has(currentTrigger?.value, 'title'))
                        ...
                        <ExecutorConfig
                            ...
                            onFinish={(value) => {
                                ...
                                currentStep?.onFinish({
                                    ...value,
                                    ...((value?.operator === currentStep?.value?.operator) && has(currentStep?.value, 'title'))
            ...
            >
                <Button type="text" onClick={validate}>
                    检查
                </Button>
                <Button
                    type="text"
                    onClick={() => {
                        const value = cloneDeep(DefaultSteps);
                        if (isControlled) {
                            if (typeof onChange === "function") {
                                onChange(value);
                            }
                        ...
                        setTimeout(() => {
                            scaleable.current?.scrollTo({
                                left: "center",
                                top:
                                    scaleable.current.container!.offsetHeight -
                                    200,
                            });
                        }, 33);
                    }}
                ...
                <Button
                    ...
                    onClick={() =>
                        scaleable.current?.scrollTo({
                            left: "center",
                            top:
                                scaleable.current.container!.offsetHeight - 200,
                        })
                    }
                />
                <input
                    ...
                    value={
                        scaleValue > 1
                            ? (25 * Math.sqrt(40 * scaleValue - 39) + 425) / 9
                            : (500 * scaleValue - 50) / 9
                    }
                    onChange={(e) => {
                        const value = parseFloat(e.target.value);
                        scaleable.current?.scaleTo(
                            value > 50
                                ? 0.00324 * Math.pow(value, 2) -
                                      0.306 * value +
                                      8.2
                                : 0.018 * value + 0.1
                        );
                    }}
                    onBlur={() => scaleable.current?.scaleEnded()}
                />
            </div> */}
