      ...
      >
        <div className={styles.head}>
          <div className={styles.title}>
            {step && stepNodes[step.id] ? stepNodes[step.id]!.index + 1 : 1}
            .&nbsp;
            {
              <>
                ...
                {/* {step?.operator ? null : (
                    <Popover
                      content={() => (
                        <div style={{ width: 200 }} onClick={stopPropagation}>
                          <Typography.Text>
                            {t("editor.step.triggerTipTitle", "什么是触发器？")}
                          </Typography.Text>
                          <br />
                          <Typography.Text>
                            {t(
                    ...
                    >
                      <QuestionCircleOutlined className={styles.titleTip} />
                    </Popover>
                  )} */}
              </>
            }
        ...
        <div
          className={clsx(
            styles.body,
            !step?.parameters?.fields?.length && styles.empty
          )}
        >
          {step?.parameters?.fields?.length ? (
            <Node action={{ ...action, title }!} t={te} />
          ) : (
            <PlusCircleFilled className={styles.addIcon} />
          )}
        </div>
        {hasError ? <ErrorPopover code={validateResult.get(step)!} /> : null}
      </div>
