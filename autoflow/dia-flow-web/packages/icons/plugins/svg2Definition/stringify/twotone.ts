...
const colorsReplacer = applyTo({
    ...
    pipe(
        ...
        toPairs,
        map(compose<ReplaceType, FlippedReplaceType, ApplyFlippedReplaceType>(apply, flip)(replace)),
        apply<(str: string) => string, (str: string) => string>(pipe)
    )
);

type ReplaceType = (pattern: RegExp, replacement: string) => (str: string) => string;

type FlippedReplaceType = (replacement: string, pattern: RegExp) => (str: string) => string;

type ApplyFlippedReplaceType = ([replacement, pattern]: [string, RegExp]) => (str: string) => string;

const duplicate = <T>(n: T): [T, T] => [clone(n), clone(n)];

export const twotoneStringify: StringifyFn = pipe(
    duplicate,
    zipWith<(asnd: AbstractNodeDefinition) => string, AbstractNodeDefinition, string>(call as any, [
        pipe(assoc("icon", __PLACEHOLDER__), JSON.stringify),
        pipe(
            prop("icon"),
            JSON.stringify,
            colorsReplacer,
            content => `function render(${__PRIMARY_COLOR__}, ${__SECONDARY_COLOR__}) { return ${content}; }`
        )
    ]),
