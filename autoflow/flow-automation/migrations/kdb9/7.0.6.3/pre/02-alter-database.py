...
ALTER_TABLE_DICT = [
    ...
    ["workflow",    "t_automation_executor",          "COLUMN",   "MODIFY",     "f_name",         "varchar(256) NOT NULL DEFAULT ''",   "节点名称"],
    ["workflow",    "t_automation_executor",          "COLUMN",   "MODIFY",     "f_description",  "varchar(1024) NOT NULL DEFAULT ''",  "节点描述"],
    ["workflow",    "t_automation_executor_action",   "COLUMN",   "MODIFY",     "f_name",         "varchar(256) NOT NULL DEFAULT ''",   "动作名称"],
    ["workflow",    "t_automation_executor_action",   "COLUMN",   "MODIFY",     "f_description",  "varchar(1024) NOT NULL DEFAULT ''",  "动作描述"],
]
...
def column_exists(conn_cursor, db_name, table_name, column_name):
    # 查询字段是否存在
    conn_cursor.execute(f"SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA='{db_name}' AND TABLE_NAME='{table_name}' AND COLUMN_NAME='{column_name}';")
    # 如果结果集中有数据，表示字段存在
